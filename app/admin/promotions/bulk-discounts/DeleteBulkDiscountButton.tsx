...
import { deleteBulkDiscountRule } from "../actions";
import { Trash2, Loader2 } from "lucide-react";

interface DeleteBulkDiscountButtonProps {
  ruleId: string;
}

export function DeleteBulkDiscountButton({ ruleId }: DeleteBulkDiscountButtonProps) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      await deleteBulkDiscountRule(ruleId);
      setShowConfirm(false);
    } catch (error) {
      console.error("Failed to delete bulk discount rule:", error);
    } finally {
      setLoading(false);
    }
  };
